...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        callsign = os.getenv("DJANGO_ADMIN_CALLSIGN", "ADMIN").strip().upper()
        email = os.getenv("DJANGO_ADMIN_EMAIL") or os.getenv("SMTP_USERNAME", "")
        password = os.getenv("ADMIN_PASSWORD", "")
        if not email or not password:
            raise CommandError("DJANGO_ADMIN_EMAIL/SMTP_USERNAME and ADMIN_PASSWORD are required")
